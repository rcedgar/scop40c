...
Rather than a one-time manual curation based on subjective choices, we preferred to construct an automated pipeline with user-settable thresholds to decouple the underlying methodology from our own judgments, enable curation of future SCOP releases, and allow other workers to set different thresholds. Amino acid sequence homology between folds was inferred by all-vs-all alignments by BLASTP and by HMMer hidden Markov models (HMMs), where one HMM was constructed for each superfamily and for each fold. Any BLASTP or HMM alignment between distinct folds and below the _E_-value threshold (1E-6 by default) causes both folds to be discarded. Folds are also discarded if the median domain length is below a threshold set to 80 by default. A domain is discarded if it is shorter than half the median length of its superfamily. Finally, we excluded both folds in a pair where three out of four of TM-align, DALI, Foldseek and Reseek reported high alignment scores for a pair of domains, with high thresholds chosen for each alignment method. Using a 3/4 consensus was designed to balance the competing goals of removing cases where high structural similarity of uncertain origin may degrade analyses based on the curated reference, reducing bias towards one algorithm, and preventing an algorithm which performs poorly on a given type of structure from having a unilateral veto.